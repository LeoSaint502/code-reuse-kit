...
> ✅ 如果你用的是 **Reasonix** agent，`/code-library-save` skill 已安装。
...
```bash
# 在你的项目目录里运行一次（只需要做一次）
python ~/code-library/scripts/install_agent_config.py --project .
```
...
- **Cursor / Copilot / Windsurf** → 在项目目录里运行一次 `python ~/code-library/scripts/install_agent_config.py --project .`，之后每次任务结束也会自动存档。
...
| Agent | 存档方式 | 用户操作 |
|-------|:--------:|:--------:|
| Reasonix | `/code-library-save` skill | 零操作 |
...
python ~/code-library/scripts/extract_from_diff.py --repo .

# 手动同步
cd ~/code-library && python scripts/sync.py

# 卸载自动配置
python ~/code-library/scripts/install_hooks.py --uninstall

# 重装/更新
python ~/code-library/scripts/install_code_library.py
...
~/code-library/
  ...
  skills/
    code-library-save.md       ← Reasonix 自动存档 skill
  docs/
